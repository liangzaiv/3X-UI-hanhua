...
ensure_pg_client() {
    ...
    echo -e "${yellow}正在安装面板内备份所需的 PostgreSQL 客户端工具（pg_dump/pg_restore）...${plain}" >&2
    case "${release}" in
        ubuntu | debian | armbian)
...
ssl_cert_issue() {
    ...
    if ~/.acme.sh/acme.sh --list 2> /dev/null | awk '{print $1}' | grep -Fxq "${domain}"; then
        ...
        if [[ -n "${acmeCertDir}" ]]; then
            ...
            echo -e "${yellow}发现 ${domain} 的 acme.sh 状态不完整（没有有效证书文件），正在清理并重新签发。${plain}"
    ...
    local WebPort=80
    prompt_or_default WebPort "请选择使用哪个端口（默认 80）：" "80" XUI_ACME_HTTP_PORT
...
prompt_and_setup_ssl() {
    ...
    if [[ "$NONINTERACTIVE" == "1" ]]; then
        case "${XUI_SSL_MODE:-none}" in
            ...
            none | "") ssl_choice="4" ;;
            *)
                echo -e "${yellow}未知的 XUI_SSL_MODE='${XUI_SSL_MODE}'，默认使用 none（HTTP）。${plain}"
                ssl_choice="4"
                ;;
        esac
    else
...
config_after_install() {
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "  1) SQLite     （默认，推荐少于 500 个客户端）"
...
EOF
                    ...
                    ensure_pg_client || echo -e "${yellow}⚠ 无法安装 pg_dump/pg_restore。在安装 postgresql-client 包之前，面板内数据库备份/还原不可用。${plain}"
                fi
            fi
            ...
            echo -e "${yellow}或 SSH 隧道已经为你处理 TLS 时才跳过。${plain}"
            ...
            if [[ "$db_choice" == "2" ]]; then
                echo ""
                echo -e "${green}面板已内置 PostgreSQL 备份与还原：${plain}"
                echo -e "  ${blue}${SSL_SCHEME}://${SSL_HOST}:${config_port}/${config_webBasePath}${plain} → Backup & Restore"
                echo -e "${yellow}  备份会下载 pg_dump .dump 文件；还原会通过 pg_restore 重新载入。${plain}"
            fi
            ...
            if [[ "$db_choice" == "2" && "$pg_local_installed" == "1" ]]; then
                ...
                echo -e "${green}═══════════════════════════════════════════${plain}"
                echo -e "${yellow}⚠ 面板会从此处读取这些凭据： ${xui_env_file}.${plain}"
                echo -e "${yellow}⚠ 请保存密码，它不会以明文存储在其他位置。${plain}"
...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "https://api.github.com/repos/liangzaiv/3X-UI-hanhua/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "https://api.github.com/repos/liangzaiv/3X-UI-hanhua/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
        ...
        curl -4fLR --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 300 -o ${xui_folder}-linux-$(arch).tar.gz https://github.com/liangzaiv/3X-UI-hanhua/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="https://github.com/liangzaiv/3X-UI-hanhua/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    curl -4fLRo /usr/bin/x-ui-temp https://raw.githubusercontent.com/liangzaiv/3X-UI-hanhua/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/liangzaiv/3X-UI-hanhua/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/liangzaiv/3X-UI-hanhua/main/x-ui.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/liangzaiv/3X-UI-hanhua/main/x-ui.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/liangzaiv/3X-UI-hanhua/main/x-ui.service.rhel > /dev/null 2>&1
                    ;;
            esac
    ...
    echo -e "${green}x-ui ${tag_version}${plain} 安装完成，现在已运行..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
